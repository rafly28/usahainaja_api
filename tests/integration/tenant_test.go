...
func TestAuthEndpoints(t *testing.T) {
	...
	// re-fetch csrf to store cookie in jar
	req, _ = http.NewRequest(http.MethodGet, server.URL+"/api/auth/csrf", nil)
...
func registerUser(t *testing.T, client *http.Client, url string, name, email, password string) string {
	...
	jar, _ := cookiejar.New(nil)
	client.Jar = jar
	csrfToken := loginAndGetCSRF(t, client, url, "", "")
...
func TestBusinessCreation(t *testing.T) {
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("expected 200 OK for create business, got %d", resp.StatusCode)
	}
...
func switchBusiness(t *testing.T, client *http.Client, url, csrfToken, code string) {
	req, _ := http.NewRequest(http.MethodPost, url+"/api/businesses/"+code+"/switch", nil)
	req.Header.Set("X-CSRF-Token", csrfToken)
	resp, err := client.Do(req)
	if err != nil {
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("expected 200 OK for switch business, got %d", resp.StatusCode)
	}
}
...
func TestCrossTenantIsolation(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, server.URL+"/api/businesses/"+bizA+"/switch", nil)
	req.Header.Set("X-CSRF-Token", csrfB)
	resp, _ := clientB.Do(req)
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusForbidden && resp.StatusCode != http.StatusNotFound {
		t.Fatalf("expected 403 or 404 for cross-tenant switch, got %d", resp.StatusCode)
	}

	// Create product in Business B
	switchBusiness(t, clientB, server.URL, csrfB, bizB)
	body := strings.NewReader(`{"name":"Product B", "selling_price":1000, "is_stock_tracked":true}`)
...
func TestProductTrackingLifecycle(t *testing.T) {
	...
	// 1. Create product
	body := strings.NewReader(`{"name":"Product C", "selling_price":1000, "is_stock_tracked":true}`)
	req, _ := http.NewRequest(http.MethodPost, server.URL+"/api/products", body)
	req.Header.Set("Content-Type", "application/json")
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("failed to create product, got %d", resp.StatusCode)
	}
	...
	// 2. Opening stock
	osBody := strings.NewReader(`{"product_code":"` + code + `", "location_code":"LOC-DEFAULT", "quantity":10, "unit_cost":500}`)
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("failed to create opening stock, got %d", resp.StatusCode)
	}
	...
	if len(invResp.Data.Items) == 0 || invResp.Data.Items[0].Quantity != "10" {
...
func TestDuplicateOpeningStock(t *testing.T) {
	...
	body := strings.NewReader(`{"name":"Product D", "selling_price":1000, "is_stock_tracked":true}`)
	req, _ := http.NewRequest(http.MethodPost, server.URL+"/api/products", body)
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-CSRF-Token", csrf)
	resp, _ := client.Do(req)
	...
	// Opening stock 1
	osBody := `{"product_code":"` + prodResp.Data.Code + `", "location_code":"LOC-DEFAULT", "quantity":10, "unit_cost":500}`
...
func TestTransactionRollback(t *testing.T) {
	...
	body := strings.NewReader(`{"name":"Product E", "selling_price":1000, "is_stock_tracked":true}`)
	req, _ := http.NewRequest(http.MethodPost, server.URL+"/api/products", body)
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-CSRF-Token", csrf)
	resp, _ := client.Do(req)
